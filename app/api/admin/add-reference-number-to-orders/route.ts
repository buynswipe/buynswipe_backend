...
export async function POST(request: Request) {
  try {
    ...
    // Add the reference_number column to the orders table using the SQL API
    ...
    if (!response.ok) {
      let errorMessage = `Failed to add reference_number column with status ${response.status}`
      try {
