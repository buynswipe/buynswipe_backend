...
// Cache for notifications to avoid repeated API calls
...
export function OptimizedNotificationBell() {
  ...
  const getNotificationIcon = useCallback((type: string) => {
    ...
    return (
      <div
        className={`w-2 h-2 rounded-full mr-3 ${iconConfig[type as keyof typeof iconConfig] || iconConfig.system}`}
      />
    )
  }, [])
  ...
  // Optimized notification loading with caching
  const loadNotifications = useCallback(async () => {
    try {
      ...
      if (error || !dbNotifications || dbNotifications.length === 0) {
        ...
        notificationsData = dbNotifications as Notification[]
      }
  ...
  const handleNotificationClick = useCallback(
    (notification: Notification) => {
      ...
      if (!notification.is_read) {
        ...
        session.then(({ data }) => {
          if (data.session) {
            const cacheKey = `notifications_${data.session.user.id}`
            const cached = notificationCache.get(cacheKey)
            if (cached) {
              const updatedData = cached.data.map((n) => (n.id === notification.id ? { ...n, is_read: true } : n))
              notificationCache.set(cacheKey, {
                data: updatedData,
  ...
  const markAllAsRead = useCallback(() => {
    ...
    session.then(({ data }) => {
      if (data.session) {
        ...
        const cached = notificationCache.get(cacheKey)
        if (cached) {
          const updatedData = cached.data.map((n) => ({ ...n, is_read: true }))
          notificationCache.set(cacheKey, {
            data: updatedData,
            timestamp: cached.timestamp,
          })
